...
public static class UpdateScript
{
        ...
        bool relaunch = true)
    {
        ...
        script.AppendLine("}");
        script.AppendLine();
        script.AppendLine("# Hashing through .NET rather than Get-FileHash: that cmdlet is missing from some");
        script.AppendLine("# Windows PowerShell environments (measured on a CI runner, where the update failed");
        script.AppendLine("# with \"The term 'Get-FileHash' is not recognized\"), and checking a download is too");
        script.AppendLine("# important to depend on a cmdlet that may not be there.");
        script.AppendLine("function Get-Sha256([string]$path) {");
        script.AppendLine("    $sha = [System.Security.Cryptography.SHA256]::Create()");
        script.AppendLine("    try {");
        script.AppendLine("        $stream = [System.IO.File]::OpenRead($path)");
        script.AppendLine("        try {");
        script.AppendLine("            return ([System.BitConverter]::ToString($sha.ComputeHash($stream))).Replace('-', '').ToLowerInvariant()");
        script.AppendLine("        } finally { $stream.Dispose() }");
        script.AppendLine("    } finally { $sha.Dispose() }");
        script.AppendLine("}");
        script.AppendLine();
        ...
        script.AppendLine("        $digest = Get-Sha256 $source");
        ...
        script.AppendLine("            $present = Get-Sha256 $target");
        ...
        script.AppendLine("        $after = Get-Sha256 $target");
